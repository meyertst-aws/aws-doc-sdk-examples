...
class Commit:
    ...
    def __init__(self, repo: str, head_ref: str, token: str):
        self.repo = repo
    ...
    def base_url_compare(self):
        return f"https://api.github.com/repos/{self.repo}/compare"
    ...
    def get_commit_diff(self) -> List[unidiff.PatchSet]:
        ...
        diffs = self.get_for_compare("v3.diff", f"/{self.head_ref}...main")
...
def comment_diagnostic_to_log(diagnostic, source_line, end_line, log_messages, http_prefix):
    ...
    log_messages.append(
        f"::error ::{message}\n    {http_path}#L{source_line}\n   {file_path}:{source_line}")
...
def get_clang_tidy_warnings(
    ...
    try:
        ...
        pass
 # 2022-11-7 Amazon commented out
    #        print(
...
def main(
 ...
 # 2022-11-7 Amazon changed start
    ...
    #    print(f"\nDiff from GitHub PR:\n{diff}\n")
        ...
        commit = Commit(repo, head_ref, token)
        diff = commit.get_commit_diff()
    else:
        print("No pull request or head reference. Unable to review.")
        return 0

    head_ref = strip_enclosing_quotes(head_ref)
    http_prefix = f"https://github.com/{repo}/tree/{head_ref}"
